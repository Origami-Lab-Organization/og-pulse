/**
 * Sessão do Pulse para o MCP de prospecção.
 *
 * Entra com as credenciais da própria pessoa usando a chave publicável — então a
 * RLS vale normalmente e o MCP enxerga só o que ela enxerga: sem `prospeccao:ler`
 * não lê nada, sem `prospeccao:editar` não escreve nada. Espelha
 * `apps/mcp-activities/src/supabase.ts`. Nunca usar service key aqui: ela bypassa a
 * RLS e, com ela, o `tenant_id`.
 *
...
const SESSION_PATH = join(homedir(), '.og-pulse', 'prospeccao-session.json');
...
export interface PulseEmployee {
  /** `employees.id` — é o que prospects e prospect_activities referenciam como autor. */
  employeeId: string;
  tenantId: string;
  nome: string;
}
