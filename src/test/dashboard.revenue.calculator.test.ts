...
import {
  ...
  type DashboardRevenueInput,
} from '@/lib/dashboardRevenueCalculator';

function input(overrides: Partial<DashboardRevenueInput> = {}): DashboardRevenueInput {
  return {
    faturamentoTotal: 0,
...
describe('calculateDashboardRevenue', () => {
  ...
  it('Margem é null (não 0%) quando não há faturamento no período', () => {
    const r = calculateDashboardRevenue(input({ faturamentoTotal: 0, personnelCostMonthly: 5_000 }));
    expect(r.margemReal).toBeNull();
  });

  it('trata monthsInPeriod < 1 como 1 mês (guarda contra divisão temporal inválida)', () => {
    const r = calculateDashboardRevenue(input({ personnelCostMonthly: 8_000, monthsInPeriod: 0 }));
