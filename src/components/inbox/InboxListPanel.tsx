...
const folderItems: { key: InboxFolder; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
...
export function InboxListPanel({
  ...
  const handleKeyDown = useCallback(
    ...
    [notifications, focusedIndex, onSelect, onToggleCheck, onArchive, onBulkAction],
  );
  ...
  return (
    <div className="w-full md:w-[400px] border-r flex flex-col shrink-0 min-h-0">
            ...
            <div className="flex md:hidden flex-1 min-w-0">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="sm" className="h-8 gap-1.5 px-2 max-w-full">
                    <ActiveFolderIcon className="h-4 w-4 shrink-0" />
                    <span className="text-sm truncate">{activeFolder?.label}</span>
                ...
                <DropdownMenuContent align="start" className="w-52">
                  {folderItems.map(({ key, label, icon: Icon }) => (
                    <DropdownMenuItem
                      ...
                      className={cn('gap-2', folder === key && 'bg-accent text-accent-foreground')}
                    >
                      <Icon className="h-4 w-4" />
                      {label}
                    </DropdownMenuItem>
      ...
      <div
        ...
        onFocus={() => { if (focusedIndex < 0 && notifications.length > 0) setFocusedIndex(0); }}
        ...
        {isLoading ? (
          ...
          notifications.map((n, index) => (
            <InboxNotificationRow
              ...
              onClick={() => { setFocusedIndex(index); onSelect(n); }}
              index={index}
