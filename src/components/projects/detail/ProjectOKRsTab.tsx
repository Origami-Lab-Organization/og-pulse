...
import { OKR_STATUS_LABELS, ProjectOKR, ProjectKeyResult } from '@/types/projectOkr';
...
export function ProjectOKRsTab({ project }: ProjectOKRsTabProps) {
  ...
  const handleEditOkr = (okr: ProjectOKR) => {
    setEditingOkr(okr);
    setOkrDialogOpen(true);
  };
  ...
  const handleAddKeyResult = (okrId: string) => {
    setSelectedOkrId(okrId);
    setEditingKr(null);
    setKrDialogOpen(true);
  };

  const handleEditKeyResult = (kr: ProjectKeyResult, okrId: string) => {
    setSelectedOkrId(okrId);
    setEditingKr(kr);
    setKrDialogOpen(true);
  };
                ...
                <div className="flex items-center gap-4 text-sm text-muted-foreground mt-2">
                  {okr.target_date && (
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        Meta: {format(new Date(okr.target_date), 'dd/MM/yyyy', { locale: ptBR })}
                      </span>
                    </div>
                  )}
              ...
              <CardContent className="pt-0">
                <div className="border-t pt-3 space-y-3">
                    ...
                    okr.key_results.map((kr, krIndex) => (
                      ...
                      >
                        ...
                        <Badge variant="outline" className={`${getStatusColor(kr.status)} text-xs`}>
                          {kr.status === 'completed' ? '✓' : kr.status === 'in_progress' ? '•' : '○'}
                        </Badge>
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                          <Button
                            ...
                            size="icon"
                            className="h-6 w-6"
                            onClick={() => handleEditKeyResult(kr, okr.id)}
                          >
                  ...
                  <Button
                    ...
                    onClick={() => handleAddKeyResult(okr.id)}
                  >
                    <Plus className="mr-2 h-4 w-4" />
