...
export const BudgetFinancialSummary = forwardRef<HTMLDivElement, BudgetFinancialSummaryProps>(
  function BudgetFinancialSummary(
    ...
    if (layout === 'footer') {
      return (
        <div ref={ref} className="border-t bg-card shadow-[0_-4px_20px_-4px_rgba(0,0,0,0.08)]">
          {/* Linha superior - informações detalhadas */}
          <div className="border-b border-border/50 px-6 py-3">
            <div className="flex flex-wrap items-start gap-6 lg:gap-10">
                ...
                <div className="grid grid-cols-[auto_auto] gap-x-3 gap-y-0.5 text-sm">
                  ...
                  {calculation.materialsTotal > 0 && (
                    <>
                      ...
                      <span className="text-right font-medium">{formatCurrency(calculation.materialsTotal)}</span>
                    </>
                  )}
                </div>
              ...
              {/* Grupo: Composição */}
              <div className="space-y-1.5">
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Composição</span>
                <div className="grid grid-cols-[auto_auto] gap-x-3 gap-y-0.5 text-sm">
                  <span className="text-muted-foreground">Desp. Adm. ({adminExpensesPercent}%)</span>
                  <span className="text-right">{formatCurrency(calculation.adminExpenses)}</span>
                  <span className="text-muted-foreground">Impostos ({taxesPercent}%)</span>
                  <span className="text-right">{formatCurrency(calculation.taxes)}</span>
                </div>
              </div>
              
              {/* Grupo: Markup Editável */}
              <div className="space-y-2 ml-auto">
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Markup</span>
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    <Label htmlFor="commission-footer" className="text-sm text-muted-foreground w-20">Comissão</Label>
                    <Input
                    ...
                    <span className="text-sm text-muted-foreground">= {formatCurrency(calculation.commission)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Label htmlFor="margin-footer" className="text-sm text-muted-foreground w-20">Margem</Label>
                    <Input
                      id="margin-footer"
                      type="number"
                    ...
                    />
                    <span className="text-xs text-muted-foreground">%</span>
                    <span className="text-sm text-muted-foreground">= {formatCurrency(calculation.netMargin)}</span>
                  </div>
                </div>
              </div>
            </div>
