    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Popover open={termOpen && !data.notice_worked} onOpenChange={o => !data.notice_worked && setTermOpen(o)}>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                disabled={data.notice_worked}
                className={cn("w-full justify-start text-left font-normal", !termDate && "text-muted-foreground", showErrors && !termDate && "border-destructive")}
              >
                <CalendarIcon className="mr-2 h-4 w-4" />
          ...
          {data.notice_worked ? (
            <p className="text-xs text-muted-foreground">Calculada automaticamente (comunicação + dias de aviso prévio) na etapa "Aviso Prévio", já que o aviso será trabalhado</p>
          ) : (
            <>
              {showErrors && !termDate && (
      ...
      {/* Fim Antecipado de Contrato — decide Art. 479 (empresa) vs Art. 480 (funcionário) */}
      {data.termination_type === 'early_contract_termination' && (
        <div className="space-y-2">
          <Label>Quem antecipou o desligamento? *</Label>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => onChange({ early_termination_initiated_by: 'company' })}
              className={`px-3 py-1 rounded-md text-sm ${data.early_termination_initiated_by === 'company' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}
            >
              Empresa (indenização, Art. 479)
            </button>
            <button
              type="button"
              onClick={() => onChange({ early_termination_initiated_by: 'employee' })}
              className={`px-3 py-1 rounded-md text-sm ${data.early_termination_initiated_by === 'employee' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}
            >
              Funcionário (desconto, Art. 480)
            </button>
          </div>
          {showErrors && !data.early_termination_initiated_by && (
            <p className="text-xs text-destructive">Selecione quem antecipou o desligamento</p>
          )}
        </div>
      )}
