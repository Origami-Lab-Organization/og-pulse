-- ─────────────────────────────────────────────────────────────────────────────
-- Reaplica o filtro `aloca_em_projetos` em get_allocation_employee_month_summary,
-- perdido por drift de schema fora de migration.
--
-- Diagnóstico (sessão de suporte, 2026-07-22): confirmado via
-- `pg_get_functiondef('public.get_allocation_employee_month_summary(uuid, integer, uuid, uuid, text)'::regprocedure)`
-- que a função hoje em produção é SECURITY DEFINER e tem um
-- `AND public.user_belongs_to_tenant(auth.uid(), p_tenant_id)` no WHERE final —
-- nenhuma dessas duas características existe em NENHUMA migration deste
-- repositório (nem na 20260619170000_complete_role_allocations_cutover.sql
-- original, nem na 20260722090000_employee_allocation_eligibility.sql que
-- adicionou `aloca_em_projetos`). Ou seja: alguém rodou um
-- `CREATE OR REPLACE FUNCTION` manual direto no banco (provavelmente ao
-- resolver o mesmo bug de visibilidade de PM que motivou a
-- 20260722130000_project_role_allocations_pm_tenant_read.sql), a partir de uma
-- versão da função anterior ao filtro de `aloca_em_projetos` — apagando-o
-- silenciosamente sem nenhum registro em migration.
--
-- Esta migration reconstrói a função a partir do que está DE FATO em produção
-- hoje (preservando SECURITY DEFINER + user_belongs_to_tenant — não é para
-- reverter esse fix, que parece intencional), reinserindo apenas
-- `AND e.aloca_em_projetos = true` no employee_scope.
--
-- Boundary violado por essa alteração manual: "Nao alterar schema Supabase sem
-- migration versionada" (.harness/boundaries.md). Reforço: qualquer ajuste
-- futuro nesta função DEVE ir por migration, nunca direto no SQL Editor —
-- senão este mesmo drift se repete.
-- ─────────────────────────────────────────────────────────────────────────────
...
WHERE e.tenant_id = p_tenant_id
  AND public.user_belongs_to_tenant(auth.uid(), p_tenant_id)
