-- Adiciona project_end_date ao retorno de get_allocation_employee_detail
-- para que o frontend calcule a editabilidade com base na end_date real
-- em vez de duration_months (coluna estática que não é recalculada quando o PM muda a end_date).
...
project_planned AS (
  SELECT
    ...
    pm.end_date AS project_end_date,
    pm.duration_months,
    pm.is_continuous,
  ...
  GROUP BY pm.project_id, pm.project_member_id, pm.name, pm.manager_name, pm.team_label,
    pm.client_name, pm.manager_id, pm.team_key, pm.start_date, pm.end_date, pm.duration_months, pm.is_continuous, month
),
project_actual AS (
  SELECT
    'project'::text AS item_type,
...
GROUP BY item_type, item_id, project_id, project_member_id, title, subtitle, client_name,
  manager_id, manager_name, team_key, team_label, project_start_date, project_end_date,
