...
BEGIN
  SELECT count(*), min(id), max(tenant_id), max(cargo)
  INTO v_match_count, v_employee_id, v_tenant_id, v_current_cargo
  FROM public.employees
  WHERE nome ILIKE 'Rafael Bruno Andrade';
  ...
  INTO v_existing_termination_id
  FROM public.employees
  ...
  IF v_termination_id IS NULL THEN
    SELECT id
    INTO v_termination_id
    FROM public.employee_terminations
    WHERE employee_id = v_employee_id
      AND termination_date = DATE '2026-04-17'
  ...
  IF v_termination_id IS NULL THEN
    ...
    VALUES (
      ...
      DATE '2026-04-17',
      'contract_end',
      'Correção histórica de desligamento PJ em 2026-04-17',
      'contract_expiration',
      0,
      false,
      'completed',
  ...
  WHERE employee_id = v_employee_id
    AND effective_from >= DATE '2026-02-18';
  ...
  SET
    ...
    decimo_terceiro = 0.000000,
    ferias = 0.000000,
    pro_labore = 0.000000,
    provisao_13 = 0.000000,
    provisao_ferias = 0.000000,
    provisao_recesso = 0.000000,
    total_monthly_cost_estimated = 0.000000,
    total_annual_cost_estimated = 0.000000,
    breakdown_json = jsonb_build_object(
      'baseAmount', 0.000000,
      'chargesAmount', 0.000000,
      'provisionsAmount', 0.000000,
      'benefitsAmount', 0.000000,
      'toolsAmount', 0.000000,
      'totalMonthlyCost', 0.000000,
      'totalAnnualCost', 0.000000,
  ...
  WHERE pmm.id = mm.id
    AND mm.month_start > DATE '2026-04-17';
