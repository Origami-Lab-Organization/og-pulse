-- Corrige dois problemas relacionados a mudanças de jornada diária:
--
-- 1. CORREÇÃO DE DADOS: versões existentes onde jornada_diaria ficou com o DEFAULT 8
--    porque createVersion não salvava o campo. Recalcula a partir de jornada_mensal,
--    que é sempre salvo corretamente (jornada_diaria = jornada_mensal / 22).
--
-- 2. RPC: get_allocation_employee_month_summary passa a ler jornada_diaria do
--    employee_versions vigente em cada mês, em vez do valor atual de employees.
--    Isso garante que mudanças mid-year (ex: 6→8h em 21/04) reflitam corretamente
--    na capacidade de cada mês.

-- ── 1. Corrige dados históricos ───────────────────────────────────────────────
UPDATE public.employee_versions
SET jornada_diaria = ROUND(jornada_mensal::numeric / 22)
WHERE ROUND(jornada_mensal::numeric / 22) <> jornada_diaria;
...
SELECT
  ...
  e.cargo,
  -- Lê jornada_diaria da versão vigente no 1º dia do mês, caindo em e.jornada_diaria se
  -- não houver versão registrada para aquele mês.
  COALESCE(
    (
