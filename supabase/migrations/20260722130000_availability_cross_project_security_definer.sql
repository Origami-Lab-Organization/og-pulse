-- ─────────────────────────────────────────────────────────────────────────────
-- Disponibilidade cross-projeto (v2.2 item 1)
--
-- Problema: get_allocation_employee_month_summary era SECURITY INVOKER, então a
-- RLS do chamador filtrava os projetos de OUTROS gerentes ao somar horas
-- planejadas. Um funcionário 100% alocado em projetos de terceiros aparecia como
-- "livre" para o GP atual (célula de mês futuro e painel de disponibilidade do
-- dialog de alocação usam esta RPC como fonte de capacidade/planejado).
--
-- Correção: SECURITY DEFINER para que a agregação enxergue TODOS os projetos do
-- tenant (fato sobre a pessoa, não sobre a visibilidade de projeto), + guarda de
-- tenant explícita: se o chamador não pertence ao tenant, a função não retorna
-- linha nenhuma (nunca soma horas de outro tenant). A RPC devolve apenas horas
-- agregadas + capacidade — sem nomes de projeto e sem valores financeiros, então
-- os projetos de terceiros permanecem anônimos (o nome só vem de
-- get_allocation_employee_detail, que continua SECURITY INVOKER e só lista os
-- projetos visíveis ao GP).
--
-- Corpo idêntico ao do cutover (20260619170000); mudam apenas a linha SECURITY e
-- a guarda de tenant no WHERE final.
-- ─────────────────────────────────────────────────────────────────────────────
