-- Adiciona suporte a Year-to-Date (YTD) no RPC get_allocation_type_kpis.
-- p_ytd_cutoff_date: última sexta-feira antes da semana atual (calculada no frontend).
-- YTD planned  = meses 1..N onde N = mês do cutoff.
-- YTD actual   = lançamentos de 01/Jan até p_ytd_cutoff_date (inclusive).
...
AS $$
  ...
  -- ── YTD (ano até última sexta-feira) ──────────────────────────────────────
  ...
  proj_planned_ytd AS (
    ...
    WHERE  pm.employee_id IS NOT NULL
      AND  EXTRACT(YEAR FROM (date_trunc('month', p.start_date)::date + ((pmm.month_number - 1) * interval '1 month')))::integer = p_year
      AND  EXTRACT(MONTH FROM (date_trunc('month', p.start_date)::date + ((pmm.month_number - 1) * interval '1 month')))::integer
             <= EXTRACT(MONTH FROM p_ytd_cutoff_date)::integer
  ),
