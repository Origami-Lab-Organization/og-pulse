-- Corrige histórico de custo e jornada da Kauany desde a admissão.
-- A fonte histórica para analytics/custos é employee_versions + snapshots cost_per_hour.
...
DECLARE
  ...
  v_current_telefone text;
  v_current_cpf text;
  v_current_status text;
  v_current_system_role text;
  v_current_is_gerente boolean;
  v_current_salario_liquido numeric;
  v_current_pro_labore numeric;
  v_current_dividendos numeric;
...
BEGIN
  ...
  SELECT
    ...
    system_role,
    is_gerente,
    COALESCE(salario_liquido, 0),
    COALESCE(pro_labore, 0),
    COALESCE(dividendos, 0)
  INTO
    v_employee_id,
    v_tenant_id,
    v_current_cargo,
    ...
    v_current_cpf,
    v_current_status,
    v_current_system_role,
    v_current_is_gerente,
    v_current_salario_liquido,
    v_current_pro_labore,
    v_current_dividendos
  FROM public.employees
  WHERE nome ILIKE 'Kauany Sebastiana Arantes';

  -- Remove versões da janela corrigida para recriar intervalos sem sobreposição.
  DELETE FROM public.employee_versions
  WHERE employee_id = v_employee_id
    AND effective_from >= DATE '2026-01-07';

  -- Fecha eventual versão anterior que atravessaria a data de admissão corrigida.
  ...
  SET
    ...
    breakdown_json = jsonb_build_object(
      ...
      'details', jsonb_build_object(
        ...
        'terceiros', 0.000000,
        'outros', 0.000000,
        'provisao13', v_clt_decimo,
        'provisaoFeriasBase', v_clt_decimo,
        'provisaoFeriasTerco', 61.111111,
        'provisaoFerias', v_clt_ferias,
        'provisaoRecesso', 0.000000,
        'fgts13', 14.666667,
        'fgtsFerias', 19.555556,
        'encargos13', 14.666667,
        'encargosFerias', 19.555556
      )
    ),
    updated_at = now()
  WHERE id = v_employee_id;

  -- Benefícios atuais: apenas VR/VA e Colab+ ativos para este caso.
  ...
  ) AS benefit(name, description, monthly_value, origin_key);

  -- Ferramentas atuais: apenas MS365 e Claude ativos para este caso.
  ...
  WHERE employee_id = v_employee_id
    AND lower(name) IN ('ms365', 'claude');
  ...
  ) AS tool(name, description, monthly_cost);

  -- Recalcula snapshots de custo planejado por mês para a Kauany.
  ...
  member_month_costs AS (
    ...
    CROSS JOIN LATERAL (
      ...
      FROM generate_series(
        mmd.month_start,
        (mmd.month_start + interval '1 month - 1 day')::date,
        interval '1 day'
      ) AS gs(day)
        ...
        AND NOT EXISTS (
          ...
          WHERE ch.tenant_id = v_tenant_id
            ...
            AND (
              ...
              OR (ch.holiday_type IN ('floating', 'one_time')
  ...
  -- Recalcula snapshots de custo realizado por lançamento para a Kauany.
  ...
  -- Valida que a reconstrução gerou exatamente as quatro versões esperadas.
  SELECT count(*)
  INTO v_match_count
