-- Rollback do escopo por projeto (PUL-201 / TD-0018): `projeto:editar` volta a valer
-- para qualquer projeto do tenant.
...
CREATE POLICY "Admins and managers can delete key result history" ON public.key_result_history
  ...
  WHERE ((kr.id = key_result_history.key_result_id) AND has_capability(auth.uid(), p.tenant_id, 'projeto:editar'::text)))));
...
CREATE POLICY "Admins and managers can delete project key results" ON public.project_key_results
  ...
  USING ((EXISTS ( SELECT 1
   FROM (project_okrs o
     JOIN projects p ON ((p.id = o.project_id)))
  WHERE ((o.id = project_key_results.okr_id) AND has_capability(auth.uid(), p.tenant_id, 'projeto:editar'::text)))));
...
CREATE POLICY "Admins and managers can delete project members" ON public.project_members
  ...
  USING ((EXISTS ( SELECT 1
   FROM projects p
  WHERE ((p.id = project_members.project_id) AND has_capability(auth.uid(), p.tenant_id, 'projeto:editar'::text)))));
...
CREATE POLICY "Admins and managers can delete project milestones" ON public.project_milestones
  ...
  USING ((EXISTS ( SELECT 1
   FROM projects p
  WHERE ((p.id = project_milestones.project_id) AND has_capability(auth.uid(), p.tenant_id, 'projeto:editar'::text)))));
...
CREATE POLICY "Admins and managers can delete project okrs" ON public.project_okrs
  ...
  USING ((EXISTS ( SELECT 1
   FROM projects p
  WHERE ((p.id = project_okrs.project_id) AND has_capability(auth.uid(), p.tenant_id, 'projeto:editar'::text)))));
...
CREATE POLICY "Admins and managers can delete project stakeholders" ON public.project_stakeholders
  ...
  USING ((EXISTS ( SELECT 1
   FROM projects p
  WHERE ((p.id = project_stakeholders.project_id) AND has_capability(auth.uid(), p.tenant_id, 'projeto:editar'::text)))));
